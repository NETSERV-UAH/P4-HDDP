...
class ExerciseTopo(Topo):
    ...
    def __init__(self, hosts, switches, links, log_dir, bmv2_exe, pcap_dir, **opts):
        ...
        # assumes host always comes first for host<-->switch linksdd SDN network nodes
        for link in links:
            if link['node1'][0] == 'h':
	    ...
	    print str(sw)+":"+str(params)+"\n"
            if "program" in params:
                switchClass = configureP4Switch(
                        ...
                        log_console=True,
                        pcap_dump=pcap_dir)
	    elif "switch_class" in params:
		if str(params['switch_class']) == "OVSSwitch":
			switchClass = OVSSwitch
		else:
			switchClass = str(params['switch_class']) 
            else:
                # add default switch
                switchClass = None
	    print "switchClass: "+str(switchClass)
        ...
        for link in switch_links:
            sw1_name = self.parse_switch_node(link['node1'])
            sw2_name = self.parse_switch_node(link['node2'])
            self.addLink(sw1_name, sw2_name, delay=link['latency'], bw=link['bandwidth'])
...
class ExerciseRunner:
    ...
    def run_exercise(self):
        ...
        self.create_network()
        c = self.net.addController('c', controller=RemoteController, ip = '127.0.0.1')
        c.start()
        self.net.start()
        sleep(1)
